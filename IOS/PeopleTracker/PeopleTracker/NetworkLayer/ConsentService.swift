...
class ConsentService {
    ...
    // Api to approve/reject consent
    func approveOrRejectConsentWithToken(consentUrl: URL,parameters: [String : Any], completion: @escaping (Result<ConsentModel, Error>) -> Void) -> Void  {
        
        let networkManager = NetworkManager.init(url: consentUrl)
        let request = networkManager.buildRequest(method: NetworkManager.Method.post, accessKey: accessKey,parameters: parameters)
        networkManager.sendRequest(request: request) { (result) in
            switch result {
            case .success(_, let data) :
