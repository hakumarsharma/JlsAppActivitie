...
class GroupService {
    ...
    func createGroup(createGroupUrl: URL, parameters: [String : Any], methodType: String,completion: @escaping (Result<GroupModel, Error>) -> Void) -> Void  {
        
        let networkManager = NetworkManager.init(url: createGroupUrl)
        let request = networkManager.buildRequest(method: NetworkManager.Method(rawValue: methodType)!, accessKey: accessKey, parameters: parameters)
...
//    }
    ...
    // Api to exitOrRemove from group
    func exitOrRemoveFromGroup(exitGroupUrl: URL,parameters: [String : Any], completion: @escaping (Result<UserModel, Error>) -> Void) -> Void{
        let networkManager = NetworkManager.init(url: exitGroupUrl)
        let request = networkManager.buildRequest(method: NetworkManager.Method.delete, accessKey: accessKey,parameters: parameters)
        networkManager.sendRequest(request: request) { (result) in
            switch result {
            case .success(_, let data) :
                do {
                    if let resultdata = data {
                        if let json = try JSONSerialization.jsonObject(with: resultdata) as? [String : Any] {
                            ...
                            if let string = String(data: data, encoding: String.Encoding.utf8) {
                                let jsonData = string.data(using: .utf8)!
                                let groupMemberData = try! JSONDecoder().decode(UserModel.self, from: jsonData)
                                completion(.success(groupMemberData))
                            }
                        }
                    }
